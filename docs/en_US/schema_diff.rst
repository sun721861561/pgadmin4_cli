...
Schema Diff API Integration
===========================

Schema Diff also provides API-first endpoints to run compare flows without
using the legacy Schema Diff React page.

Local test page
---------------

Open the local manager page:

* ``/schema_diff/manager``

The page is intended for local testing and includes:

* Source and target PostgreSQL connection forms (pre-filled local defaults)
* Mode selector for object diff vs migration script
* Compare options (ignore owner/whitespace/tablespace/grants)
* Response viewer

API endpoints
-------------

Use authenticated ``POST`` requests with JSON body.

* ``/schema_diff/api/object_diff``:
  Return diff details for a single object.
* ``/schema_diff/api/migration_sql``:
  Return full migration SQL for all compared objects.
* ``/schema_diff/api/compare``:
  Backward-compatible endpoint. Uses ``operation`` in payload.
...
.. code-block:: json

   {
     ...
     "compare_options": {
       ...
       "ignore_tablespace": false,
       "ignore_grants": false
     }
   }

For ``/schema_diff/api/object_diff``, add:
...
.. code-block:: json

   {
     "single_object": {
       "object_type": "table",
       "object_name": "public.my_table"
     }
   }

You can also identify object rows by ``source_oid`` and ``target_oid``.

For ``/schema_diff/api/migration_sql``, optionally add:

.. code-block:: json

   {
     "include_comparison_rows": false
   }

Connection behavior
-------------------

The API creates ad-hoc source/target server connections per request and
cleans them up after completion. Credentials are used only for that request.

Local PostgreSQL test setup
---------------------------

1. Create two local databases (for example ``schema_src`` and ``schema_tar``).
2. Apply different DDL to each database.
3. Open ``/schema_diff/manager`` in pgAdmin.
4. Fill local credentials and choose the two databases.
5. Run:
   * ``single_object_diff`` to inspect one object.
   * ``full_migration_sql`` to generate synchronization SQL.
